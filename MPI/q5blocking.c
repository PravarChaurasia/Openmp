// Blocking communication is done using MPI_Send() and MPI_Recv().
// These functions do not return (i.e., they block) until the communication is finished

#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h>

#define MASTER 0 // root
#define TAG 42   // tag size to differntiate on msges while sending and receving(communication).

int main(int argc, char* argv[])
{
    MPI_Init(&argc, &argv);
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);

    int count = 0;
    
    srand(time(NULL)^rank);  // wont store time immediately when fn of time called 
    // parallely
    int mynum = rand()%20;
    printf("Processor %d with number %d\n", rank, mynum);

    MPI_Barrier(MPI_COMM_WORLD);

    while (size > 1)  // more then one process
    {
        if (rank == MASTER)   // rank=0
        {
            printf("\nIteration %d with %d processes active\n", ++count, size);
            
        }

        if(size%2!=0) //no of process is odd  
        {
            if(rank == 0)
            {//send middle element to 0 position
                int x;
                MPI_Recv(&x, 1, MPI_INTEGER, size/2, TAG, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
                mynum+=x;  // rooot on receiving msg from every process and adding intergers
            }
            if(rank == size/2 ) // middle
            {
                MPI_Send(&mynum, 1, MPI_INTEGER, 0, TAG, MPI_COMM_WORLD);
            }
            
            for (int i = 0; i < size/2; i++)  // p(i+n/2) sends value to add.
            {
                if(rank == i)
                {
                    int x;
                    MPI_Recv(&x, 1, MPI_INTEGER, i + size/2 + 1, TAG, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
                    mynum+=x;
                }
                if(rank == i + size/2 + 1)
                {
                    MPI_Send(&mynum, 1, MPI_INTEGER, i, TAG, MPI_COMM_WORLD);
                }
            }
        }
        else //size is even
        {
            for (int i = 0; i < size/2; i++)
            {
                if(rank == i)
                {
                    int x;
                    MPI_Recv(&x, 1, MPI_INTEGER, i + size/2, TAG, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
                    mynum+=x;
                }
                if(rank == i + size/2)
                {
                    MPI_Send(&mynum, 1, MPI_INTEGER, i, TAG, MPI_COMM_WORLD);
                }
            }
        }

        size/=2;

    }

    if(rank == MASTER)
        printf("\nArray Sum: %d\n", mynum);

    MPI_Finalize();
    return 0;
}