// Non-blocking communication is done using MPI_Isend() and MPI_Irecv(). These function return immediately (i.e., they do not block) even if the communication is not finished yet.
    ...
    while (size > 1)
    {
        ...
        if(size%2!=0) //size is odd
        {
            ...
            {//send middle element to 0 position
                ...
                MPI_Wait(&oddRecv, &status); // Waits for an MPI request to complete
                mynum+=x;
            }
            if(rank == size/2 )
            {
